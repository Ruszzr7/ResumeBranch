...
test('work and project child ordering stays in the module-order dialog and saves layout-only order maps', () => {
  ...
  assert.ok(previewSource.includes('contentBlockOrderByEntry'))
  assert.equal(previewSource.includes("const resumeResponse = await fetch('/save_resume'"), false)
  assert.equal(previewSource.includes("emit('resume-updated', resumeCandidate)"), false)
  ...
  assert.ok(previewSource.includes("experienceContentOrderSnapshot(props.data, 'work_experience', nextLayout)"))
