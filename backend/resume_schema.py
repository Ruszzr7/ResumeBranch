...
class WorkExperience(ResumeModel):
    ...
    entry_id: str | None = Field(
        default=None,
        description="稳定内部经历标识；由服务端生成，Agent 不得修改",
    )
    ...
    content_blocks: list[ProjectContentBlock] = Field(
        default_factory=list,
        description="工作简介、工作职责及零个或多个其他工作内容语义块；其他工作内容可带自定义标签或留空，空标签正文仍显示和导出；内容块保存顺序与排版显示顺序分离；每种内容均可按原文或用户要求使用段落、分点或编号形式；工作经历不使用项目技术栈角色",
    )
...
class ProjectExperience(ResumeModel):
    ...
    content_blocks: list[ProjectContentBlock] = Field(
        default_factory=list,
        description="技术栈、项目简介、项目职责及零个或多个其他项目内容语义块；其他项目内容可带自定义标签或留空，空标签正文仍显示和导出；内容块保存顺序与排版显示顺序分离，语义角色与段落、分点、编号形式相互独立",
    )
...
def ensure_resume_identity(data: dict) -> dict:
    """Validate a resume and assign stable IDs exactly at a write boundary.

    IDs are deliberately not generated by ``validate_resume_data`` because
    validation is also used for hashing and repeated reads.  Assigning them
    only when data is accepted for persistence keeps digests deterministic.
    """
    result = validate_resume_data(data)
    seen_entry_ids: set[str] = set()
    for root in ("work_experience", "project_experience"):
        for entry in result.get(root, []):
