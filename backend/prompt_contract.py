...
4. 【内容块边界与分类顺序】先按原文视觉结构划分候选内容组：栏目标题、段落/分点/编号、缩进、对齐、空行和连续性共同确定边界；视觉边界优先于语义猜测。出现明确的项目简介或项目职责标题时，先按标题建立内容组，再用视觉边界确定其覆盖范围。项目简介后、在同一视觉层级且没有新标题的连续内容，默认属于项目职责；只有视觉层级明确不同、脱离该组的内容才另建 generic。若整个经历没有任何明确的项目简介/项目职责标题，全部内容归入 generic，不根据语义自行补造 introduction 或 responsibilities。
5. 【内容完整性】项目简介后的内容先按上述视觉范围归入项目职责，不因单个词语或句子含义擅自拆分；出现新的明确标题或清晰的视觉边界时再结束当前组并建立下一个 generic 或语义组。所有可见内容必须原样进入某个 content_blocks 且只能出现一次，不能省略；无法确定边界时保留为 generic。
6. 教育经历栏目下、下一个顶层栏目之前的无标题分点，必须进入“教育经历补充”；只有原文存在独立的“荣誉/奖项/奖学金”标题时才进入主要荣誉。不能仅凭条目语义把教育经历内部内容改分到主要荣誉。
7. 只有原文确实存在独立栏目时，才把语言、证书、荣誉或论文分别映射到对应模块；一个可见栏目混合多种内容时必须整体保留为自定义栏目，不能按语义拆散。
8. 用户没有要求修改的内容和布局必须原样保留；任何不确定的归类都保留原文，不猜测、不补写。粗体只依据原文视觉证据，不能因为英文、缩写、技术名词、数字、百分比或看起来重要就推断加粗。段落或分点开头出现完整粗体片段并紧接冒号/中文冒号或其他分隔符（例如“**重点内容**：XXXXX”）时，必须优先检查并保留开头实际粗体片段；这是高优先级的局部粗体证据，但只标记冒号前实际粗体范围，不延伸到后文。
""".strip()
