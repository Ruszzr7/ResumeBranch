...
【经历内容结构契约】
1. 工作经历的 content_blocks 只允许 introduction（项目简介）、responsibilities（项目职责）、generic（普通内容）三种语义角色；项目经历额外允许 tech_stack（技术栈），不能创建其他语义角色或任意自定义标题。项目内部只有原文明确出现“技术栈/技术选型/使用技术/技术工具”标题时，才填写 tech_stack；只有原文明确出现“项目简介/项目背景/项目概述/项目说明”或“项目职责/主要职责/个人职责/负责内容”等语义标题时，才填写对应 label；generic 的 label 必须为空。
2. 四种角色的默认展示形式固定为：技术栈和项目简介用段落，项目职责用编号，普通内容用分点。仅当原文明确使用另一种段落/分点/编号形式时才保留原形式；列表条目只保存正文，不重复写序号或圆点，标题为空表示保留内容但不显示标题。
3. 项目经历的 content_blocks 固定按技术栈、项目简介、项目职责、普通内容的顺序组织；缺少某一块时跳过，不因排序补造内容。技术栈、项目简介和项目职责的语义标题在渲染时与内容同级、各有一个分点；项目职责标题下的多条内容使用编号，普通内容与两者保持同级并使用分点。位于分点内部的局部粗体前缀必须留在同一个正文条目中，不能提升为 label 或新内容块。
4. 【内容块边界与分类顺序】先按原文视觉结构划分候选内容组：栏目标题、段落/分点/编号、缩进、对齐、空行和连续性共同确定边界；视觉边界优先于语义猜测。项目内部明确的技术栈标题及其正文属于 tech_stack，并在项目简介之前；项目简介标题及其正文属于 introduction；只有项目简介正文结束后的后续内容才进入职责候选区。此后的连续内容默认属于 responsibilities，先采用第一个连续视觉组作为职责组；只有后续内容出现明确不同的视觉组（例如缩进层级、分点位置或编号/分点结构发生清晰变化）时，才另建 generic。若整个经历没有任何明确的项目简介/项目职责/技术栈标题，全部内容归入 generic，不根据语义自行补造语义角色，也不能仅凭出现 Python、Java 等技术名词就创建 tech_stack。
...
6. 【结构示例】如果项目经历依次出现“技术栈：Python、FastAPI”、项目简介段落、四个职责分点和一个明显不同视觉组的其他说明，应分别生成 tech_stack、introduction、responsibilities、generic，并按固定顺序输出。若项目简介正文之后有四个分点，其中前三个缩进和分点位置一致，第四个明显没有缩进，则前三个归 responsibilities，第四个归 generic；判断依据是视觉组差异，而不是“第四条”这一位置本身。项目职责默认使用编号，普通内容默认使用分点。
