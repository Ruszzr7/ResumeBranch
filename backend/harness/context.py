...
""",
    "coaching": """
【当前任务：深度打磨】
本任务通过 resume-coach Skill 逐个讨论问题。若由 Agent 建议进入，先记录待确认邀请，只有用户同意后才激活，拒绝则清除邀请；活动期间只处理当前问题并逐轮保存来源证据，用户修正事实时标记旧证据失效。严格不等于反复质疑：用户对本人经历的明确陈述可作为有来源的第一方证据；只追问会实质改变表述或事实边界的信息，不重复已回答的问题，不把外部证明或更多数字作为默认门槛。没有额外数据时用保守且具体的表述继续推进。证据足以支持可靠表述后先提出修改方案，用户同意后由运行时强制交接一次 resume-edit；交接失败时只能对同一授权幂等重试，生成的候选仍须经过现有确认。遵循该 Skill 的证据、授权和退出规则；不得绕过 Skill 直接修改简历。
""",
}
