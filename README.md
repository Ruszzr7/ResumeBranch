...
| Windows 单用户安装 | 从 GitHub Releases 下载 `ResumeBranch-Setup-v1.2.0-x64.exe` 并安装 | 仅单用户 | Windows 本地直接使用 |
...
普通 Windows 用户应从 GitHub Releases 下载 `ResumeBranch-Setup-v1.2.0-x64.exe` 并运行安装程序。安装包已经包含冻结后的单用户版程序、前端生产文件、私有 Python 运行环境及依赖、Nginx、Poppler 和 PDF 渲染浏览器。目标电脑不需要另外安装 Python、Node.js、npm、Docker、MySQL 或 Inno Setup，也不会修改系统 `PATH`。
...
生成的文件位于 `output/installer/ResumeBranch-Setup-v1.2.0-x64.exe`。具体打包说明见 [Windows 安装包构建说明](packaging/README.md)。
